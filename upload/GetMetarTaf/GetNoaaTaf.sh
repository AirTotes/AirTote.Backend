...
SOURCE_URL='https://aviationweather.gov/adds/dataserver_current/current/tafs.cache.csv'
SOURCE_FILE_NAME='tafs.cache.csv'
LOCAL_CACHE_MODIFIED_DATE_RECORD_FILE_NAME='taf_noaa.date.log'
OUTPUT_CSV_FILE_NAME='taf_jp.csv'
DATE_CMD='date'
ERR_HEAD_LOG_DIR='taf_cache_err_log'
...
# キャッシュをサーバから取得
# 先頭の不要な行を削除する
# CSVファイルが大きすぎるため、カラム数を減らす
curl "$SOURCE_URL" 2> /dev/null | sed '1,6d' | python3 $MINIMIZE_PY > $SOURCE_FILE_NAME
